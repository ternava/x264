...
x264_opts="" # "--no-cabac --no-mbtree" 

x264_args="x264 $x264_opts $ivideo -o o1.mkv" 

# first, let's breath command line arguments for x264 
# the idea is to fix argc and argv right in the main function of x264.c, see replace_args.py for more details
# not an idea solution, but I can't make it work properly directly within gcov 
python replace_args.py "$x264_args"
